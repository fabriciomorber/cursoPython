...
# Função para calcular a média de avaliação de um filme
avarage_ratings = lambda movie_name: sum(ratings[movie_name])/len(ratings[movie_name])
...
recommend_movie = lambda movie_name: f"Recomendo assistir {movie_name} com média de {avarage_ratings(movie_name)}"

print(f"Média de Avaliação do filme The Matrix: {avarage_ratings('The Matrix')}")
